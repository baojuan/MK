...
- (id)initWithState:(BOOL)isSimple delegate:(id)delegate
{
    ...
    if (self) {
        ...
        self.editButton.frame = CGRectMake(width * 0, 0, width, height);
        [self addSubview:self.editButton];
        
        
        self.talkButton = [self buttonWithSelector:@selector(talkButtonClick:) imagename:@"webtalk"];
        self.talkButton.frame = CGRectMake(width * 1, 0, width, height);
        ...
        self.centerLabel = [[UILabel alloc] initWithFrame:RECT(width * 2, 0, width, height)];
        ...
        self.shareButton.frame = CGRectMake(width * 3, 0, width, height);
        [self addSubview:self.shareButton];
        
        self.downloadButton = [self buttonWithSelector:@selector(downloadButtonClick:) imagename:@"save_task"];
        self.downloadButton.frame = CGRectMake(width * 4, 0, width, height);
