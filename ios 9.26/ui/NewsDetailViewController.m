...
@interface NewsDetailViewController ()<UITableViewDataSource,UITableViewDelegate,MobiSageBannerDelegate,NewsDetailCellDelegate>
...
- (void)setHeadView
{
    ...
    navigationView = [[MKNavigationView alloc] initWithTitle:self.navTitle rightButtonImage:IMAGENAMED(@"articleCollect") ForPad:YES ForPadFullScreen:YES];
...
{
    self.tableView = [[UITableView alloc] initWithFrame:RECT(0, navigationView.frame.size.height, SCREEN_WIDTH, SCREEN_HEIGHT - navigationView.frame.size.height - 35 - 50)];
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        self.tableView.frame = RECT(0, navigationView.frame.size.height, SCREEN_HEIGHT, SCREEN_WIDTH - navigationView.frame.size.height - 35 - 90);
    }
...
{
    UIView *view = [[UIView alloc] initWithFrame:RECT(0, 0, self.view.frame.size.width, 140)];
...
//    }
    if ([[dict objectForKey:@"classe"] isEqualToString:@"p"]) {
        ...
        return [[dict objectForKey:@"textHeight"] floatValue] + 17;
    }
