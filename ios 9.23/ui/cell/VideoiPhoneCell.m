...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier ForSmall:(BOOL)small ForRight:(BOOL)right
{
    ...
    if (self) {
        ...
        self.videoImageView = [[UIImageView alloc] initWithFrame:RECT(10, 15, 102, 58)];
        ...
        */
        self.titleLabel = [[UILabel alloc] initWithFrame:RECT(self.videoImageView.frame.size.width + self.videoImageView.frame.origin.x + 10, self.videoImageView.frame.origin.y - 3, SCREEN_WIDTH - 10 - (self.videoImageView.frame.size.width + self.videoImageView.frame.origin.x + 10), 40)];
        ...
        imageView.frame = RECT(self.titleLabel.frame.origin.x, self.titleLabel.frame.size.height + self.titleLabel.frame.origin.y + 5, imageView.frame.size.width, imageView.frame.size.height);
        ...
        if (!right) {
            self.titleLabel.frame = RECT(self.videoImageView.frame.size.width + self.videoImageView.frame.origin.x + 10, self.videoImageView.frame.origin.y, 470 - 10 - (self.videoImageView.frame.size.width + self.videoImageView.frame.origin.x + 10), 40);
            self.dateLabel.frame = RECT(470 - 10 - 100, self.playCountLabel.frame.origin.y, 100, self.playCountLabel.frame.size.height);
        }
        if (small) {
            self.titleLabel.frame = RECT(self.videoImageView.frame.size.width + self.videoImageView.frame.origin.x + 10, self.videoImageView.frame.origin.y, 320 - 10 - (self.videoImageView.frame.size.width + self.videoImageView.frame.origin.x + 10), 40);
        }
