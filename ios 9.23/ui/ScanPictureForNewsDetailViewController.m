//
//  ScanPictureForNewsDetailViewController.m
//  MKProject
//
...
@interface ScanPictureForNewsDetailViewController ()<MobiSageBannerDelegate>
@property(nonatomic, strong) MobiSageBanner* mobiSageBanner;

@end

@implementation ScanPictureForNewsDetailViewController
{
...
//        UIImageView * imageView = [[UIImageView alloc] initWithFrame:RECT((scroll.frame.size.width - 361 / 2.0) / 2.0, (scroll.frame.size.height - 344 / 2.0) / 2.0, 361 / 2.0, 344 / 2.0)];
    ...
    [self setBottomView];
    [self bannerId];
    self.scrollView.contentOffset = POINT(self.scrollView.frame.size.width * indexForImage, 0);
...
- (void)setBottomView
{
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        ...
        toolBarForPad.centerLabel.text = [NSString stringWithFormat:@"%d/%d",indexForImage + 1,[self.dataArray count]];
        ...
        toolBar.centerLabel.text = [NSString stringWithFormat:@"%d/%d",indexForImage + 1,[self.dataArray count]];
