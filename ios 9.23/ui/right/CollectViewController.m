...
- (void)getData
{
    ...
    NSDictionary *dict = [NSDictionary dictionaryWithDictionary:[[NSUserDefaults standardUserDefaults] objectForKey:COLLECT_USERDEFAULT]];
...
- (void)setHeadView
{
    ...
    [navigationView.rightButton setImage:IMAGENAMED(@"commentConfirm") forState:UIControlStateSelected];
...
- (void)backButtonClick
{
    if (isedit) {
        ...
        hud.labelText = @"保存中，请稍后";
        hud.removeFromSuperViewOnHide = YES;
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            NSMutableDictionary *ddForUserdefault = [[NSMutableDictionary alloc] init];
            NSString *tt;
            for (int i = 0; i < [_dataArray count]; i ++) {
                tt = [appdelegate.itemArray objectAtIndex:i];
                NSMutableArray *aa = [NSMutableArray array];
                for (ArticleModel * ii in [_dataArray objectAtIndex:i]) {
                    [aa addObject:[ModelExChangeDictionary modelChangeToDictionary:ii]];
                }
                [ddForUserdefault setObject:aa forKey:tt];
            }
            
            [[NSUserDefaults standardUserDefaults] setObject:ddForUserdefault forKey:COLLECT_USERDEFAULT];
...
- (void)setTableViewProperty
{
    ...
    noImageView.image = IMAGENAMED(@"nocollection");
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSMutableArray *array = [NSMutableArray arrayWithArray:[_dataArray objectAtIndex:segmentController.selectedSegmentIndex]];
    [array removeObjectAtIndex:indexPath.row];
    [_dataArray replaceObjectAtIndex:segmentController.selectedSegmentIndex withObject:array];
