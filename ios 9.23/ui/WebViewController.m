...
@interface WebViewController ()<UIWebViewDelegate,MobiSageBannerDelegate>
...
{
    webView = [[UIWebView alloc] initWithFrame:RECT(0, navigationView.frame.size.height, SCREEN_WIDTH, SCREEN_HEIGHT - navigationView.frame.size.height - 35 - 50)];
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        webView.frame = RECT(0, navigationView.frame.size.height, SCREEN_HEIGHT, SCREEN_WIDTH - navigationView.frame.size.height - 35 - 90);
    }
...
- (void)setBottomView
{
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        ...
        toolBar = [[WebViewToolBar alloc] initWithState:NO delegate:self];
